...
class TestFollowAPI:
    ...
    def test_follow_create(
        ...
        assert (
            follow_count + 1 == Follow.objects.count()
    ...
    def test_follow_search_filter(
        ...
        assert (
            len(response.json())
            == follow_user.filter(following=user_2).count()
        ), (
        ...
        assert (
            len(response.json())
            == follow_user.filter(following=another_user).count()
        ), (
            "Проверьте, что при GET запросе с параметром `search` на `/api/v1/follow/` "
            "возвращается результат поиска по подписке"
        )
